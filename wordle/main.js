...
const allWordsArr=[];
let a = "";
for(var i=0; i<allWordsText.length; i++){
    if(a.length==5){
        allWordsArr.push(a);
        a="";
    }
...
const letters = ["a", "á", "b", "c", "č", "d", "ď", "e", "é", "ě", "f", "g", "h", "ch", "i", "í", "j", "k", "l", "m", "n", "ň", "o", "ó", "p", "q", "r", "ř", "s", "š", "t", "ť", "u", "ú", "ů", "v", "w", "x", "y", "ý", "z", "ž"];

const state = {
    secret: "panda", // allWordsArr[Math.floor(Math.random()*allWordsArr.length)],
    grid: Array(6)
        .fill()
        .map(() => Array(5).fill('')),
    currentRow: 0,
